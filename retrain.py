...
print("Generating Regional Dataset (Sus, Lavale Village, Nande)...")
np.random.seed(42)
data_size = 3500
...
# NEW: 0 = Sus/Baner, 1 = Lavale Village, 2 = Nande
route_region = np.random.choice([0, 1, 2], p=[0.4, 0.4, 0.2], size=data_size)
...
for h, m, w, steep, blocked, region in zip(hours, months, is_weekday, is_steep_route, is_blocked, route_region):
    # --- TRAFFIC LOGIC (Based on Graph 1 & 2) ---
    is_rush = (8 <= h <= 10) or (17 <= h <= 20)
    is_monsoon = 6 <= m <= 9
    is_summer_break = 4 <= m <= 5
    
    if w == 1 and is_rush and not is_summer_break:
        if region == 1: cong = 'High' # Lavale Village sees massive spikes
        elif region == 2: cong = 'Medium' # Nande has lower peaks
        else: cong = 'High'
        ...
        cong = 'Low' # Weekends and Summer are flat
    else: 
        cong = 'Medium'
        
    # --- RISK LOGIC (Based on Graph 5) ---
    if region == 1 and is_monsoon and cong == 'High': 
        risk = 'Critical' # Lavale Village has huge accident spikes in Monsoon
    elif region == 2:
        risk = 'Normal' # Nande is historically much safer
    ...
    else: spd = np.random.uniform(20, 30)                   
    if steep == 1: spd *= 0.75 

    # --- AQI LOGIC (Based on Graph 4) ---
    base_aqi = 60
    if m in [11, 12, 1, 2]: base_aqi += 45
    elif is_monsoon: base_aqi -= 25
    
    if region == 1 and cong == 'High': aqi = base_aqi + np.random.uniform(80, 100) # Lavale hits 160+
    elif region == 2 and cong == 'High': aqi = base_aqi + np.random.uniform(20, 40) # Nande caps at ~100
...
print("Training Upgraded 6-Variable Models (Lavale/Nande integrated)...")
...
print("Success! .pkl files updated for Sus, Lavale, and Nande regions.")